...
namespace WinFormsApp1
{
    public partial class ulke : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            LoadTable(dataGridView1, "SELECT * FROM BSMSMEGEN003", getColumnHeaders());
        }

        private void ulke_Load(object sender, EventArgs e)
        {
            // Form yüklendiğinde yalnızca sütun başlıklarını getir
            LoadTable(dataGridView1, "SELECT * FROM BSMSMEGEN003 WHERE 1=0", getColumnHeaders());
        }
        ...
        {
            // dilEkle formunu aç
            ulkeEkle ulkeEkleForm = new ulkeEkle();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                string ulkeAdi = selectedRow.Cells["COUNTRYTEXT"].Value.ToString();

                // dilGuncelleme formunu oluştur ve bilgileri aktar
                ulkeGuncelleme ulkeGuncellemeForm = new ulkeGuncelleme();
                ulkeGuncellemeForm.SetUlkeBilgileri(firmaKodu, ulkeKodu, ulkeAdi);

                // Formu modal olarak göster
                ulkeGuncellemeForm.ShowDialog();
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                DialogResult dialogResult = MessageBox.Show("Bu ülke kaydını silmek istediğinizden emin misiniz?",
                                                            "Silme Onayı",
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    string connectionString = "Data Source=ZEYBASARAN;Initial Catalog=SMEDEMO;Integrated Security=True;";
                    string query = "DELETE FROM BSMSMEGEN003 WHERE COMCODE = @FirmaKodu AND COUNTRYCODE = @UlkeKodu";
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        try
                        {
                            ...
                            using (SqlCommand command = new SqlCommand(query, connection))
                            {
                                ...
                                command.Parameters.AddWithValue("@UlkeKodu", ulkeKodu);
                                ...
                                if (rowsAffected > 0)
                                {
                                    MessageBox.Show("Ulke kaydı başarıyla silindi.");
