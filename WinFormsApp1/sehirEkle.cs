...
namespace WinFormsApp1
{
    public partial class sehirEkle : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string selectedUlkeKodu = cmbUlkeKodu.SelectedValue.ToString();
            // Veritabanı bağlantı dizesi
            string connectionString = "Data Source=ZEYBASARAN;Initial Catalog=SMEDEMO;Integrated Security=True;";

            // Insert sorgusu (Bu örnek sadece Firma Kodu ekliyor, eklemek istediğiniz diğer alanları da ekleyebilirsiniz)
            string query = "INSERT INTO BSMSMEGEN004 (COMCODE, CITYCODE, CITYTEXT, COUNTRYCODE) VALUES (@FirmaKodu, @SehirKodu, @SehirAdı, @UlkeKodu)";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@SehirKodu", sehir_kodu_txt.Text);
                        command.Parameters.AddWithValue("@SehirAdı", sehir_adi_txt.Text);
                        command.Parameters.AddWithValue("@UlkeKodu", selectedUlkeKodu);
