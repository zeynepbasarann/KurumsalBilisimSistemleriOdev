...
{
    public partial class rotaTipiGuncelleme : Form
    {
        public rotaTipiGuncelleme()
        {
            InitializeComponent();
        }

        // Form yüklendiğinde firma kodlarını ComboBox'a yükler
        private void rotaTipiGuncelleme_Load(object sender, EventArgs e)
        {
        ...
        public void SetRotaTipiBilgileri(string firmaKodu, string rotaTipiKodu, string rotaTipiAdi, bool Pasif)
        {
            ...
            rota_tipi_txt.Text = rotaTipiKodu;
            rota_aciklamasi_txt.Text = rotaTipiAdi;
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            string yenirotaTipiKodu = rota_tipi_txt.Text;
            string yenirotaTipiAdi = rota_aciklamasi_txt.Text;
            bool yeniPasifDurum = chkPasif.Checked;

            // Veritabanına güncelleme işlemini yapalım
            string connectionString = "Data Source=ZEYBASARAN;Initial Catalog=SMEDEMO;Integrated Security=True;";
            string query = "UPDATE BSMSMEROT001 SET DOCTYPETEXT = @RotaTipiAdi ,ISPASSIVE = @Pasif  " +
                "WHERE COMCODE = @FirmaKodu AND DOCTYPE = @RotaTipiKodu";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@RotaTipiKodu", yenirotaTipiKodu);
                        command.Parameters.AddWithValue("@RotaTipiAdi", yenirotaTipiAdi);
