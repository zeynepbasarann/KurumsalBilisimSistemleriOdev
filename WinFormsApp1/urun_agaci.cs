...
{
    public partial class urun_agaci : Form
    {
        public urun_agaci()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            LoadTable(dataGridView1, "SELECT * FROM BSMSMEBOM001", getColumnHeaders());
        }

        private void urun_agaci_Load(object sender, EventArgs e)
        {
            // Form yüklendiğinde yalnızca sütun başlıklarını getir
            LoadTable(dataGridView1, "SELECT * FROM BSMSMEBOM001 WHERE 1=0", getColumnHeaders());
        }
        ...
        private Dictionary<string, string> getColumnHeaders()
        {
            ...
            {
                { "COMCODE", "Firma Kodu" },
                { "DOCTYPETEXT", "Ürün Ağacı Adı" },
                { "DOCTYPE", "Ürün Ağacı Tipi" },
        ...
        {
            // dilEkle formunu aç
            urunAgaciEkle urunAgaciEkleForm = new urunAgaciEkle();
            urunAgaciEkleForm.ShowDialog();
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                string urunAgaciKodu = selectedRow.Cells["DOCTYPE"].Value.ToString();
                string urunAgaciAdi = selectedRow.Cells["DOCTYPETEXT"].Value.ToString();
                bool Pasif = Convert.ToBoolean(selectedRow.Cells["ISPASSIVE"].Value);

                // dilGuncelleme formunu oluştur ve bilgileri aktar
                urunAgaciGuncelleme urunAgaciGuncellemeForm = new urunAgaciGuncelleme();
                urunAgaciGuncellemeForm.SetUrunAgaciBilgileri(firmaKodu, urunAgaciKodu, urunAgaciAdi, Pasif);

                // Formu modal olarak göster
                urunAgaciGuncellemeForm.ShowDialog();
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    string connectionString = "Data Source=ZEYBASARAN;Initial Catalog=SMEDEMO;Integrated Security=True;";
                    string query = "DELETE FROM BSMSMEBOM001 WHERE COMCODE = @FirmaKodu AND DOCTYPE = @urunAgaciKodu";
