...
{
    public partial class isMerkeziGuncelle : Form
    {
        public isMerkeziGuncelle()
        {
            InitializeComponent();
        }

        // Form yüklendiğinde firma kodlarını ComboBox'a yükler
        private void isMerkeziGuncelleme_Load(object sender, EventArgs e)
        {
        ...
        public void SetIsMerkeziTipiBilgileri(string firmaKodu, string isMerkeziTipiKodu, string isMerkeziTipiAdi, bool Pasif)
        {
            ...
            is_merkezi_tipi_txt.Text = isMerkeziTipiKodu;
            is_merkezi_aciklamasi.Text = isMerkeziTipiAdi;
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            ...
            string yeniisMerkeziTipiKodu = is_merkezi_tipi_txt.Text;
            string yeniisMerkeziTipiAdi = is_merkezi_aciklamasi.Text;
            bool yeniPasifDurum = chkPasif.Checked;

            // Veritabanına güncelleme işlemini yapalım
            string connectionString = "Data Source=ZEYBASARAN;Initial Catalog=SMEDEMO;Integrated Security=True;";
            string query = "UPDATE BSMSMEWCM001 SET DOCTYPETEXT = @IsMerkeziTipiAdi ,ISPASSIVE = @Pasif  " +
                "WHERE COMCODE = @FirmaKodu AND DOCTYPE = @IsMerkeziTipiKodu";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@IsMerkeziTipiKodu", yeniisMerkeziTipiKodu);
                        command.Parameters.AddWithValue("@IsMerkeziTipiAdi", yeniisMerkeziTipiAdi);
